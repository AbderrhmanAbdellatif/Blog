...
namespace ProgrammersBlog.Mvc.Controllers
{
    public class CommentController : Controller
    {
        ...
        public async Task<JsonResult> Add(CommentAddDto commentAddDto)
        {
            ...
            {
                var result = await _commentService.AddAsync(commentAddDto);// db ekliyorum
                if (result.ResultStatus==ResultStatus.Success) // eger db basilir bir sekilde olursa
                {
                    var commentAddAjaxViewModel = JsonSerializer.Serialize(new CommentAddAjaxViewModel
                    {
                        CommentDto = result.Data,// dbdei ekledigini veriyi bana getir
                        CommentAddPartial = await this.RenderViewToStringAsync("_CommentAddPartial", commentAddDto) // ayni zamanda bunu partil viwe at
